...
/** 想定外エラーを必ずログに残す（メッセージ・スタック） */
function logError(context: string, error: unknown) {
  if (error instanceof Error) {
    console.error(`[start-hanchan] ${context}:`, error.message, error.stack);
  } else {
    console.error(`[start-hanchan] ${context}:`, error);
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      console.error("[start-hanchan] Server misconfigured: missing env");
    ...
    if (roomError) {
      logError("rooms select", roomError);
    ...
    if (seatsError) {
      logError("room_seats select", seatsError);
  ...
  const { data: hanchan, error: hanchanError } = await supabase
    ...
    if (roomUpdateError) {
      logError("rooms status update", roomUpdateError);
    ...
    try {
      ...
      logError("broadcastPublicUpdate", e);
      // 開始自体は成功しているので続行
    }
